IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'Vistas_GestionAlternativas') AND type in (N'P', N'PC'))
DROP PROCEDURE Vistas_GestionAlternativas
GO
-- =============================================
-- Author:		Solver
...
-- Description:
-- =============================================
CREATE PROCEDURE Vistas_GestionAlternativas
(
...
/*************************** Variables De Prueba **********************/
--declare @ServerHostPath varchar(512),@ModuleId varchar(4), @UsuarioCreacion int,@dateFrom datetime,@dateEnd datetime,@NoReclamo varchar(50),@Cliente varchar(512),@Producto varchar(512),@Servicio varchar(512),@FromView varchar(50)
...
select	distinct
		...
		,@ServerHostPath + '/Pages/Modules/Reclamos/Admin/FrmAdminAlternativaReclamo.aspx?ModuleId=' + @ModuleId + '&IdAlternativa=' + cast(alternativa.IdAlternativa as varchar(18)) + '&from=' + @FromView
...
from	TBL_ModuloReclamos_Alternativas alternativa with(nolock)
		...
		inner join TBL_Admin_Usuarios responsable with(nolock)
			on alternativa.IdResponsable = responsable.IdUser
		inner join TBL_Admin_Usuarios autor with(nolock)
			on alternativa.CreateBy = autor.IdUser
...
where	alternativa.FechaAlternativa >= @dateFrom
		and alternativa.FechaAlternativa <= @dateEnd
