IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'Vistas_ReclamosPorTipo') AND type in (N'P', N'PC'))
DROP PROCEDURE Vistas_ReclamosPorTipo
GO
-- =============================================
-- Author:		Solver
...
-- Description:
-- =============================================
CREATE PROCEDURE Vistas_ReclamosPorTipo
(
	@dateFrom datetime
	,@dateEnd datetime
...
select	distinct
		...
		,@ServerHostPath + '/Pages/Modules/Reclamos/Admin/FrmReclamo.aspx?ModuleId=' + @ModuleId + '&IdReclamo=' + cast(reclamo.IdReclamo as varchar(18)) + '&from=rectipo'
