...
namespace Application.MainModule.SqlServices.Services
{
    public class PedidosEmpacorServices : IPedidosEmpacorServices
    {
        ...
        {
            var pedido = new PedidoClientes(_sql);
            return pedido.GetPedidoWorkFlowByIdPedido(idPedido);
        }

         public string EstadoPedido(string idPedido)
         {
             var pedido = new PedidoClientes(_sql);
             return pedido.EstadoPedido(idPedido);
         }

         public bool VerificarSuministros(string sp)
         {
             var pedido = new PedidoClientes(_sql);
             return pedido.VerificarSuministros(sp);
         }

         public DataTable GetResumenPedidos(string idPedido)
         {
             var pedido = new PedidoClientes(_sql);
             return pedido.GetResumenPedido(idPedido);
         }

         public string RetornarNombreClienteByIdPedido(string idPedido)
         {
             var pedido = new PedidoClientes(_sql);
             return pedido.RetornarNombreClienteByIdPedido(idPedido);
         }
        #endregion
