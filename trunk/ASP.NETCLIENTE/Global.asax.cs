...
namespace ASP.NETCLIENTE
{
    public class Global : HttpApplication
    {
        ...
        private const string ErrorPageLocation = "~/FrmError.aspx?error={0}";
        ...
        void Application_Error(object sender, EventArgs e)
        {
            if (Context != null && !Context.IsCustomErrorEnabled)
            {
                if (IsMaxRequestExceededException(Server.GetLastError()))
                    Server.Transfer(string.Format(ErrorPageLocation, "403"), false);
                else
                    Server.Transfer(string.Format(ErrorPageLocation,"999"), false);
            }
        }
        ...
        #region Error que sobrepasa el limite del tamaño de archivo

        const int TimedOutExceptionCode = -2147467259;

        static bool IsMaxRequestExceededException(Exception e)
        {
            Exception main;
