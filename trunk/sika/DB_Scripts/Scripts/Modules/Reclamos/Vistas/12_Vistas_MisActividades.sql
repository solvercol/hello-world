IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'Vistas_MisActividades') AND type in (N'P', N'PC'))
DROP PROCEDURE Vistas_MisActividades
GO
-- =============================================
-- Author:		Solver
...
-- Description:
-- =============================================
CREATE PROCEDURE Vistas_MisActividades
(
...
select	distinct
		...
		,@ServerHostPath + '/Pages/Modules/Reclamos/Admin/FrmAdminActividadReclamo.aspx?ModuleId=' + @ModuleId + '&IdActividad=' + cast(actividad.IdActividad as varchar(18)) + '&from=misact'
...
where	actividad.CreateBy = @UsuarioCreacion
		and actividad.Fecha >= @dateFrom
		and actividad.Fecha <= @dateEnd
