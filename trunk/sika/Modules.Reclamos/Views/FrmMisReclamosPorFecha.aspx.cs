...
namespace Modules.Reclamos.Views
{
    public partial class FrmMisReclamosPorFecha : ViewPage<MisReclamosPorFechaPresenter, IMisReclamosPorFechaView>, IMisReclamosPorFechaView
    {
        ...
        protected void BtnConfirmNuevoReclamo_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect(string.Format("../Admin/FrmAddReclamo.aspx?ModuleId={0}&tr={1}&from=misfecha", ModuleId, TipoReclamo));
            }
            else
            {
                Response.Redirect(string.Format("../Admin/FrmAddReclamo.aspx?ModuleId={0}&tr={1}&cat={2}&gruinf={3}&from=misfecha",
